exports.run = async (client, message, args) => {
    if (!args[0])
        return message.channel.send(`Please provide the name of the twitter account to monitor. Example: ${process.env.prefix}twitter tohur`);

    if (args[0].toLowerCase().includes('twitter.com'))
        return message.channel.send(`please only put the name of the twitter account: ${process.env.prefix}twitter tohur`);
