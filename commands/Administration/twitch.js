exports.run = async (client, message, args) => {
    if (!args[0])
        return message.channel.send(`Please provide the name of the channel to monitor. Example: ${process.env.prefix}twitch tohur`);

    if (args[0].toLowerCase().includes('twitch.tv'))
        return message.channel.send(`please only put the name of the channel example: ${process.env.prefix}twitch tohur`);
