...
exports.run = async (client, message, args) => {
    if (!args[0]) {
        ...
        message.channel.send(`= Command List =\n\n[Use ${process.env.prefix || client.config.prefix}help <commandname> for details]\n
            ${client.commands.map(c => `${process.env.prefix || client.config.prefix}${c.info.name}${' '.repeat(longest - c.info.name.length)} :: ${c.info.description}`).join('\n')}`, {code:'asciidoc'});
    } else {
        let command = args[0];
